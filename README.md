...
Going through it to refactor also turned up a bug: the force of
...
- **`annuity_maths.py`** is used to store formulas.
...
- **`financial_maths_calculator.py`** is the interactive CLI. Handles
...
conversions) are preserved from the original version.
The main formulas (an, andue, sn, Ian, Dan, i^(p), d^(p))
were checked against standard identities during this rewrite.
